# Project: dungeon_gen AI-lib
# Makefile configurations

DBGFLAGS	=	-O0 -g -D__DEBUG__
#DBGFLAGS	=	-O3 -g
#General flags
CFLAGS		+=	-Wall -shared $(DBGFLAGS)
LDFLAGS		+=	-ldarnit -Wl,-soname,ai_lib.so

#Makefile tools
RM		=	rm -Rf
MKDIR		=	mkdir -p


ifeq ($(strip $(OS)), Windows_NT)
	#Windows specifics
	LIB	=	$(TOPDIR)/win32-i386.ai
else 
ifeq ($(BUILDFOR), WIN32)
	#Windows specifics, for cross compiling to windows
	LIB	=	$(TOPDIR)/win32-i386.ai
	CC	=	i586-mingw32msvc-gcc
	STRIP	=	i586-mingw32msvc-strip
	AR	=	i586-mingw32msvc-ar
else
ifeq ($(BUILDFOR), WIN64)
	#Windows64 specifics, for cross compiling to windows
	LIB	=	$(TOPDIR)/bin/win64-amd64.ai
	CC	=	x86_64-w64-mingw32-gcc
	STRIP	=	x86_64-w64-mingw32-strip
	AR	=	x86_64-w64-mingw32-ar
else
ifeq ($(strip $(SBOX_UNAME_MACHINE)), arm)
	#Maemo specifics
	LIB	=	$(TOPDIR)/bin/linux-armv7.ai
	CFLAGS	+=	-fPIC
else
ifneq (,$(findstring -DPANDORA, $(CFLAGS)))
	#Pandora specifics
	LIB	=	$(TOPDIR)/bin/linux-armv7.ai
	CFLAGS	+=	-fPIC
else
ifneq (,$(findstring -DGCW_ZERO, $(CFLAGS)))
	#GCWZero specifics
	LIB	=	$(TOPDIR)/bin/gcwzero-mips32.ai
	CFLAGS	+=	-fPIC
else
ifeq ($(BUILDFOR), RPI)
	#RaspberryPi specifics
	LIB	=	$(TOPDIR)/bin/linux-armv6hf.ai
	CFLAGS	+=	-fPIC
else
	#Linux defaults
	UNAME	=	`uname -m`
ifneq (,$(findstring x86_64, $(UNAME)))
	UNAME	=	i386
endif
	LIB	=	$(TOPDIR)/bin/linux-$(UNAME).ai
	CFLAGS	+=	-fPIC
endif
endif
endif
endif
endif
endif
endif
